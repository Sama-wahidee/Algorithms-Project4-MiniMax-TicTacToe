...
public class OnePersonMinMaxGame1 {
	char cPlayer;
	Label[][] labels;
	Stage primaryStage;
	int round;
	...
	public OnePersonMinMaxGame1(Stage primaryStage, Label[][] labels, int Trounds, char user, boolean userStartsFirst,
			Label roundL, Label XscourL, Label OscourL, Label possibility) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				...
				if (labels[i][j].getText().equals(' ' + "")) {
					...
					this.possibility.setText(possibility.toString());
					labels[i][j].setText(' ' + "");
					if (moveVal > bestVal) {
						move.row = i;
						move.column = j;
